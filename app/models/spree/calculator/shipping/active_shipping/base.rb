    ...
    module ActiveShipping
      class Base < ShippingCalculator
        ...
        def country_weight_error? package
          ...
          raise Spree::ShippingError.new("#{I18n.t('spree.shipping_error')}: The maximum per package weight for the selected service from the selected country is #{max_weight} ounces.") unless valid_weight_for_package?(package, max_weight)
        end
        ...
        def retrieve_rates(origin, destination, shipment_packages)
          ...
          if [::ActiveShipping::ResponseError].include?(e.class) && e.response.is_a?(::ActiveShipping::Response)
            ...
            if params.has_key?('Response') && params['Response'].key?('Error') && params['Response']['Error'].key?('ErrorDescription')
