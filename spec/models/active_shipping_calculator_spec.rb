...
describe Spree::Calculator::Shipping do
  ...
  describe 'available' do
    context 'when rates are available' do
      ...
      it 'uses zero as a valid weight for service' do
  ...
  describe 'available?' do
    ...
    it 'does not return rates if the weight requirements for the destination country are not met' do
  ...
  describe 'compute' do
    ...
    context 'when the cache is warm' do
      it 'checks the cache first before finding rates' do
    ...
    context 'with valid response' do
      it "returns rate based on calculator's service_name" do
        expect(subject).to eq 9.99
      end

      it 'includes handling_fee when configured' do
        Spree::ActiveShipping::Config.set(handling_fee: 100)
        expect(subject).to eq 10.99
      end

      it 'returns nil if service_name is not found in rate_hash' do
    ...
    context 'with invalid response' do
      ...
      it 'raises a Spree::ShippingError' do
  ...
  describe 'service_name' do
    it 'returns description when not defined' do
